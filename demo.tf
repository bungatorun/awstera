...
resource "null_resource" "nullmount" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/bungatorun/awstera.git  /var/www/html"
    ]
  }
}
...
resource "aws_s3_bucket" "websitebucketdemo411" {
  bucket = "websitebucketdemo411"
  ...
  tags = {
    Name = "websitebucketdemo411"
    Environment = "Prod"
  }

 provisioner "local-exec" {
    command = "git clone https://github.com/bungatorun/awstera.git web-server-image"
 }
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.websitebucketdemo411]
 bucket = "websitebucketdemo411"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.websitebucketdemo411.bucket_regional_domain_name
